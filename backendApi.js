var request = require('request');
 var API_URL = "https://ba690617.us-south.apigw.appdomain.cloud/api";
//var url = "https://ba690617.us-south.apigw.appdomain.cloud/api";
var url="";

function getPatientInfo(url) {
	return new Promise(function(resolve, reject) {
		request("https://ba690617.us-south.apigw.appdomain.cloud/api/entries" , function (error, response, body) {
	 		if (!error && response.statusCode == 200) {

				body = JSON.parse(body);
                console.log(body);
				console.log(body.entries[0].name);
	    		var patientInfo = {
					      name: body.entries[0].name,
					      age: body.entries[0].name,
					      street: body.entries[0].name,
					      city: body.entries[0].name,
					      zipcode: body.entries[0].name
				};
